...
class DanmuDB(DataBase):
    ...
    def query_room(self, roomid: int, start_time: int = 0, stop_time: int = 0):
        flag = False
        if not stop_time:
            stop_time = time.time()
            flag = True
        sql = f'SELECT TIME,USERNAME,UID,MSG,MSG_TYPE,MSG_PRICE FROM DANMU WHERE '
        sql += (f'ROOM = {roomid} AND ' if roomid else '') + f'TIME >= {start_time} AND TIME < {stop_time}'
